...
class GraphSAGEEmbedding(nn.Module):
    """
    GNN-based embedding layer using GraphSAGE for user and item embeddings.
    """
    def __init__(self, num_nodes, embedding_dim, num_layers=2):
        super(GraphSAGEEmbedding, self).__init__()
        self.embedding = nn.Embedding(num_nodes, embedding_dim)
        self.gnn_layers = nn.ModuleList([GraphSAGE(embedding_dim, embedding_dim) for _ in range(num_layers)])

    def forward(self, edge_index, node_features=None):
        if node_features is None:
            node_features = self.embedding.weight  # Initialize with raw embeddings

        x = node_features
        for gnn_layer in self.gnn_layers:
            x = F.relu(gnn_layer(x, edge_index))  # Apply each GraphSAGE layer with ReLU activation
        return x


class DINWithGraphSAGE(BaseModel):
    def __init__(self, 
                 feature_map, 
                 num_nodes,
                 edge_index,
                 model_id="DINWithGraphSAGE", 
                 gpu=-1, 
                 dnn_hidden_units=[512, 128, 64],
                 dnn_activations="ReLU",
                 attention_hidden_units=[64],
                 ...
                 net_regularizer=None,
                 **kwargs):
        super(DINWithGraphSAGE, self).__init__(feature_map,
                                               model_id=model_id, 
                                               gpu=gpu, 
                                               embedding_regularizer=embedding_regularizer, 
                                               net_regularizer=net_regularizer,
        ...
        self.gnn_embedding_layer = GraphSAGEEmbedding(
            num_nodes=num_nodes,
            embedding_dim=embedding_dim
        )
        self.edge_index = edge_index  # Graph edges
    ...
    def forward(self, inputs):
        ...
        # Update feature embeddings with GNN
        gnn_embeddings = self.gnn_embedding_layer(self.edge_index)
        feature_emb_dict = {field: gnn_embeddings[X[field].long()] for field in self.feature_map}
        ...
        feature_emb = torch.cat(list(feature_emb_dict.values()), dim=-1)
        y_pred = self.dnn(feature_emb)
